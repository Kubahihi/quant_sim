...
def _auto_login_for_smoke_tests() -> int | None:
    ...
    if (
        os.environ.get("QUANT_SIM_TEST_AUTO_LOGIN") != "1"
        or not os.environ.get("PYTEST_CURRENT_TEST")
        or resolve_environment() != "test"
    ):
        return None
...
def _generate_export_artifact(result: Dict[str, Any], artifact_name: str) -> bytes:
    """Build one requested export without requiring a Streamlit run context."""
    ...
    if artifact_name == "csv":
        return export_portfolio_data_csv(report_payload)
    if artifact_name == "json":
        return export_full_report_json(report_payload)
    if artifact_name != "pdf":
        raise ValueError(f"Unsupported export artifact: {artifact_name}")
    ...
    try:
        ...
        return generate_pdf_report(report_payload, pdf_figures).getvalue()
    finally:
...
def _render_export_actions(
    ...
    def deferred_artifact(artifact_name: str):
        def generate() -> bytes:
            return _generate_export_artifact(analysis_result, artifact_name)
    ...
    if not compact:
        ...
        status_cols[0].metric("PDF Report", "On demand")
        status_cols[1].metric("CSV Export", "On demand")
        status_cols[2].metric("JSON Export", "On demand")

    export_col1, export_col2, export_col3 = st.columns(3)
    with export_col1:
        st.download_button(
            "Download PDF Report",
            data=deferred_artifact("pdf"),
            file_name=f"portfolio_report_{timestamp_suffix}.pdf",
            mime="application/pdf",
            help="Generated when clicked: charts, score, simulation, and commentary.",
            on_click="ignore",
            use_container_width=True,
    ...
    with export_col2:
        st.download_button(
            "Download Data CSV",
            data=deferred_artifact("csv"),
            file_name=f"portfolio_data_{timestamp_suffix}.csv",
            mime="text/csv",
            help="Generated when clicked for spreadsheet work.",
            on_click="ignore",
            use_container_width=True,
    ...
    with export_col3:
        st.download_button(
            "Download Full JSON",
            data=deferred_artifact("json"),
            file_name=f"portfolio_full_report_{timestamp_suffix}.json",
            mime="application/json",
            help="Generated when clicked for automation or archiving.",
            on_click="ignore",
            use_container_width=True,
...
def _render_portfolio_tracker_tab() -> None:
    ...
    if st.button("Refresh Live Valuation", key="portfolio_refresh_live", use_container_width=True):
    ...
    if "portfolio_live_holdings" not in st.session_state:
        st.caption("Live prices load on demand. Select Refresh Live Valuation when needed.")
